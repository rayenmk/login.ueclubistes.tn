...
(async function init(){
  ...
  const countBy=(key)=>Object.entries(subs.reduce((a,s)=>{const k=s[key]?.name||"Non défini";a[k]=(a[k]||0)+1;return a},{}));
  ...
  $("#recentRows").innerHTML=subs.slice(0,8).map(s=>`<tr><td>${escapeHtml(s.nom)}</td><td>${escapeHtml(s.prenom)}</td><td>${escapeHtml(s.numero_abonnement)}</td><td>${escapeHtml(s.faculties?.name||"—")}</td><td>${escapeHtml(s.zones?.name||"—")}</td><td>${statusBadge(s.status)}</td></tr>`).join("")||`<tr><td colspan="6" class="empty">Aucun abonné</td></tr>`;
})();
