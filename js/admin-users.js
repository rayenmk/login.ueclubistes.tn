...
function render(){
 $("#adminRows").innerHTML=admins.map(a=>`<tr><td>${escapeHtml(a.display_name||"—")}</td><td>${escapeHtml(a.email||"—")}</td><td>${statusBadge(a.is_active===false?"INACTIVE":"ACTIVE")}</td><td>${escapeHtml(new Date(a.created_at).toLocaleDateString("fr-TN"))}</td><td class="actions"><button class="icon-btn" title="Modifier" onclick="editAdmin('${a.id}')">✎</button><button class="icon-btn" title="Activer/Désactiver" onclick="toggleAdmin('${a.id}')">${a.is_active===false?"▶":"⏸"}</button><button class="icon-btn" title="Réinitialiser mot de passe" onclick="resetAdmin('${a.id}')">↻</button><button class="icon-btn danger" title="Supprimer" onclick="deleteAdmin('${a.id}')">⌫</button></td></tr>`).join("")||'<tr><td colspan="5" class="empty">Aucun administrateur</td></tr>';
}
...
window.resetAdmin=async id=>{const p=prompt("Nouveau mot de passe (12 caractères minimum, majuscule, minuscule, chiffre, spécial) :");if(!p)return;try{await callFn("admin-reset-password",{user_id:id,new_password:p});showAlert("#pageAlert","Mot de passe réinitialisé.")}catch(e){showAlert("#pageAlert",e.message,"error")}};
window.deleteAdmin=async id=>{if(!confirm("Supprimer définitivement cet administrateur ?"))return;try{await callFn("admin-delete-admin",{user_id:id});await loadAdmins();showAlert("#pageAlert","Administrateur supprimé.")}catch(e){showAlert("#pageAlert",e.message,"error")}};
